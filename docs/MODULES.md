...
| `rise_cv` | in | cv | (2026-09-14) The same 1 V/oct law on the **rise only**. Sums in octaves with `rate_cv`; the per-slope total is what gets clamped to ±5. Maths' per-channel jack next to its "both". Block-mean, collapsed to mono. |
| `fall_cv` | in | cv | Likewise for the **fall only**. Velocity through a [cv_scale](#cv_scale) into here is the classic — hard hits ring longer (or shorter; flip the sign) — with one honest caveat: like `rate_cv`, a `(V, F)` source is collapsed to mono by **summing** the slots, so that patch is one-note-at-a-time. Per-voice rates are a queued follow-on. |
...
* **Velocity-dependent decay** — `midi_input.velocity_cv → cv_scale → fall_cv` with `rate_cv` left for tempo: hard hits ring longer, and the rise stays put. Monophonic playing only until per-voice rates land (the CV is summed across slots).
