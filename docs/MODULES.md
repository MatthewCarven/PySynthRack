...
| `clock.swing_cv` | `0.5` (`swing_cv_depth`) | swing (fraction of a period, the knob's unit) | `clip(swing + d·mean cv, 0, 0.75)`, block-mean in float64, a non-finite mean read as 0; **latched at each even period start** and held through the odd period that follows, so a moving CV never moves a pulse already in flight — one rising edge per period, always; mono (a `(V, F)` source is averaged) |
...
| [`clock`](#clock) | Modulation | `reset`,`run` (gate), `bpm_cv`,`swing_cv` (cv) → `out` (gate) |
...
(the loop is linear, so plucks superpose — exactly, with `carry` on,
the default; see below). Every burst is seeded per
...
relocks the pitch and, with `carry` off, clears the allpass state, and
that clear alone steps the output by 7–18% of the ring's amplitude
(measured), an
...
what it was before the input existed.

**Velocity colour** (2026-09-20). A real pick's *spectrum* follows the
velocity too — a gentle stroke is duller as well as quieter — and
`vel_color` (0…1, default 0) is how much of that the string does. The
effective colour of a hit is `clamp(color + vel_color · (vel − 1), 0, 1)`,
computed at the trigger edge from the **same latched velocity** the
burst is scaled by, so it is per voice and per hit (a `(V, F)` `vel`
colours each string from its own row) and never moves mid-note. The
formula is anchored at velocity **1.0** on purpose: a full hit — or an
unpatched `vel` — is exactly `color` whatever the knob says, so
`vel_color` cannot change the sound of a patch with no velocity source
(bit-exact, pinned); it only ever *dulls* a softer hit, in proportion
(at `vel_color` 1 and `color` 0.8 a 0.3-velocity pick is coloured 0.1;
a hit above 1.0 brightens, to the clamp). Measured at C4, `color` 0.8,
`vel_color` 1: the first 50 ms of a 0.3-velocity hit has a spectral
centroid of 3.5 kHz against 5.8 kHz at full velocity. Only the exciter's
lowpass sees it — the loop is untouched, so the ring-down is the same
length whatever the pick sounded like (t60 identical at `damping` 0; at
...
(Measured 2026-09-22, default flipped 2026-09-24.) Every hit relocks
the pitch. With `carry` on it keeps the loop's fractional-delay
(allpass) state through the hit, so the relock is bit-identical to the
block-mean pitch follow the string does between hits anyway: the step
at the hit is **exactly 0**, and a re-pluck is **exact
superposition** — two hits minus one hit equals the second hit alone
to one float32 ulp (3e-08), the property this module has always
claimed. Untick it for the old behaviour: every hit **clears** that
state. Measured with a **zero** burst, so the clear is alone in the
frame, the clear steps the output by **51%** of the ring's amplitude
...
less even re-attack. Tuning is identical either way (G4 −0.95 ct,
C5 +0.84 ct, to the milli-hertz), so is the peak, and 200 rapid
re-plucks thrown four octaves around stay finite and bounded both
ways — the loop's allpass always has |c| < 1, so a carried state
decays rather than accumulating. **The clear bought nothing and cost
half the ring**, so it is off by default. A patch that stores
`"carry": false` keeps the old sound — including any patch saved from
the app while the old default stood (2026-09-22..24), since the app
saves every param; tick it to hear the new one. The examples store no
`carry` (bar `pluck_touch.json`, which ticks it), so every example
that re-plucks a ringing string moved with the flip.
...
| `carry` | `true` | tickbox | Keep the loop's allpass (fractional-delay) state through a hit, so a re-pluck is exact superposition on the ringing string (the step at the hit is exactly 0, measured). Untick for the old behaviour (the default until 2026-09-24): every hit clears it — a ~51%-of-the-ring step and a slightly harder re-attack. |
...
shuffle on the master clock itself, with a **`swing_cv`** jack (2026-09-22)
to breathe it.
...
| `swing_cv` | in | cv | Shuffle, `clip(swing + swing_cv_depth · cv, 0, 0.75)` — swing units per unit, block-mean. **Latched at each even period start**: the beat and its offbeat share one value, so a moving CV can never split, move or lose a pulse already in flight. |
...
| `swing_cv_depth` | `0.5` | 0…1 | **Swing units per CV unit** on `swing_cv` (0.5 = a CV of +1 adds half a period of shuffle). 0 disables the input without unpatching it. |
...
**Swing.** The [`clock_divider`](#clock_divider)'s convention, on the master clock: `swing` is a fraction of the period, and every *second* pulse — the offbeat — is late by that much, keeping its width; the *even* pulses (the downbeat and every other one) sit exactly where the straight clock puts them, sample for sample. Mechanically it is a phase offset on the odd periods: the clock keeps the parity of the period it is in (state, since the integer part of the phase is lost at the block-end wrap) and on an odd period the gate is `swing ≤ phase < swing + pulse_width` instead of `phase < pulse_width`. Two consequences fall out for free: a fraction of the phase is a fraction of the *current* period, so `bpm_cv` swings with the tempo; and a `reset` (or a `run` rise) restarts the parity with the phase — the pulse on the reset sample is an even, straight one, and the count starts over from it. Measured at the default 8 Hz with `swing` 0.33: odd edges 1819 samples late (= round(0.33 × 5512.5)), even edges untouched, pulse widths identical, 64- and 512-sample blocks agreeing to the sample. The ceiling: an odd pulse is cut one sample before the next even edge, so a late pulse can *never* swallow the downbeat — past `swing + pulse_width = 1` (the slider's top with the default duty) the odd pulse is shorter than the even one, never longer, and every downbeat still rises. Downstream, a divider on a swung clock counts *edges*, and edge parity is pulse parity, so its `/2`, `/4`, `/8` fire on even pulses only — a straight kick under swung hats from one clock — and since 2026-09-22 its gate **lengths** hold steady too: the [`clock_divider`](#clock_divider) takes them from its own *average* period instead of the last (alternating) interval, so a divided gate on a swung clock is usable as a gate, not only as a trigger. At 0 the straight clock is the same code path, bit-exact.

**Swing on a jack.** `swing_cv` moves the shuffle: the effective swing is `clip(swing + swing_cv_depth · mean cv, 0, 0.75)`, the mean taken in **float64** (a float32 mean of a *constant* CV drifts an ulp with the block size, and this one sets an edge) and a non-finite mean read as 0 — so a slow LFO breathes the groove from straight to triplet and back. The swing is the phase offset the odd periods are read at, so a value that changed while an odd pulse was *in flight* would drag that pulse's rising edge — forward into a *second* edge, or past the period's end into no edge at all. So it is **latched**: the block's value takes effect at the first *even* period start in the block and is held from there (the value in force before that start is carried in state). An even period and the odd one after it therefore always share one swing value — the beat and its offbeat — and the pulse in flight keeps the value its period began with. Measured: a 0.5 Hz LFO on `swing_cv` over a 10-second render emits **exactly the straight clock's 80 edges**, whatever the depth; the *even* edges land on the straight clock's own samples at both 64- and 512-sample blocks; the *odd* edges, being a block-mean away, sit up to **44 samples (1 ms)** apart between those block sizes, and the count is identical — the honest block-size story for a moving block-rate CV. A restart edge is an even period start, so a `reset` latches the current value too. Unpatched, or at `swing_cv_depth` 0, the render is the knob-only clock bit-exact.

**Patching.** `clock.out → sequencer.clock`. See `examples/sequencer_melody.json`. A slow, wide-pulse `clock` into `run` is a play/hold transport (`examples/clock_transport.json`: two bars on, one bar off); a bar-rate `clock` into `reset` *and* the sequencer's `reset` nails the phrase to the bar line; a [drift](#drift) into `bpm_cv` at a small depth (0.02–0.05) is a tempo that breathes. Swung sixteenth hats over a straight kick from the same clock: `examples/clock_swing.json`. A slow [lfo](#lfo) into `swing_cv` is the groove breathing: `examples/clock_swing_breathe.json`.
...
- `pluck_touch.json` — the whole touch: the same accent loop into a
  ...
  (mean 0.69×, correlation +0.70 with the velocity). `carry` is **on** (stored
  explicitly; it is the default now too), so each re-pluck superposes exactly
  on the string still ringing instead of stepping it — untick it to hear the
  difference the clear makes.
