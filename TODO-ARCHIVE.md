# PySynthRack — Roadmap archive (completed work)

Moved verbatim from TODO.md on 2026-07-03 to keep the live list short.
Everything here is DONE (or explicitly closed/deferred as noted). Entries
keep their follow-up notes — the recurring ones are hoisted into TODO.md's
"Follow-up threads" section; the rest live only here.
...
## Later / wishlist — shipped entries
...
- [x] **Meter module (`meter`)** — shipped 2026-06-30. A level indicator you patch any audio signal into: `in` passes through to `out` untouched, and the node shows the recent peak in dBFS on a fixed −90..0 bar (fixed so two meters are directly comparable — e.g. mic vs file player). Matthew chose a dedicated module over auto-meters on every jack, and dB floored at −90. Peak envelope (instant attack / `_METER_DECAY=0.985` slow fall) computed on the audio thread for block-rate latency; `snapshot_audio_levels()` feeds the UI, keys pre-created in compile() so the snapshot needs no lock. 18 tests in `tests/test_meter.py` (+ headless UI check); suite 595 (+18 mido). Example `examples/meter_levels.json`. Follow-ups: ~~peak-hold tick; switchable RMS mode; stereo/2-channel meter; clip indicator at 0 dBFS~~ — **all four shipped 2026-07-02** (see below).

- [x] **Meter follow-ups ×4** — shipped 2026-07-02. The Meter grew all four flagged upgrades in one pass: **stereo** (optional `in_r` → `out_r`; unpatched = the old mono meter bit-identically, patched = a second independent bar — drop it inline on a chorus/reverb stereo pair), **RMS mode** (`mode` combo peak/rms; rms = sqrt of a ~300 ms EMA of the block mean-square, loudest-voice-wins on 2D input so zero-padded slots don't dilute), **peak-hold tick** (sits exactly at the recent peak ~1.5 s, then falls at the `release` rate; peak-driven in both modes), and a **clip lamp** (any |sample| ≥ 1.0 lights it ~2 s, auto-clear, sample-counted so block-size independent). UI: each channel is now a drawlist (bar fill + tick + lamp + dB text) instead of a progress bar; R bar auto-shows/hides with `in_r`. New `snapshot_audio_meters()` GUI hook (per-channel `(level, hold, clip)` triples, same no-lock pre-created-keys discipline); `snapshot_audio_levels()` kept for back-compat. 27 new tests (48 total in `tests/test_meter.py`, incl. a bit-identical-to-old-formula guard + headless UI check). Example `examples/meter_stereo_master.json`. Follow-ups: stereo-link option (both bars share one scale peak); K-weighting / LUFS-ish mode; numeric peak readout on hover; clip counter.

- [x] **Distortion (`distortion`)** — shipped 2026-07-02. The rack's first nonlinear stage (flagged as the missing food group): a drive pedal with `soft` (normalised tanh), `hard` (clip) and `tube` (asymmetric, even harmonics, internally DC-blocked) curves, post low-pass `tone`, `level`, latency-compensated `mix`, and per-sample `drive_cv`. **4× oversampled** through a new streaming polyphase pair (`_Oversampler4`, 65-tap linear-phase FIR, per-voice zi, integer 16-sample base-rate latency) so curve harmonics are filtered off before they can alias — built as shared infrastructure for any future nonlinear stage. 27 tests in `tests/test_distortion.py` incl. an alias-suppression measurement. Example `examples/distortion_drive.json`. Follow-ups: expose the tube bias; pre/post EQ voicing; an oversampling on/off economy switch if profiled hot.

- [x] **Waveshaper (`waveshaper`)** — shipped 2026-07-02, stacked on the Distortion's oversampling plumbing. A wavefolder: `triangle` (exact geometric reflection, one vectorised mod, identity below the rails) and `sine` (smooth sin(π/2·u)) folds, `symmetry` pre-fold offset for even harmonics (DC blocker engages only while nonzero), per-sample `fold_cv` (the LFO-swept fold IS the classic patch), latency-compensated `mix`. 23 tests in `tests/test_waveshaper.py`. Example `examples/waveshaper_fold_drone.json`. Follow-ups: `wrap` mode (jump to the opposite rail — harsh digital flavour); fold_cv attenuverter is already covered by CVScale; a post low-pass if patches keep reaching for one.

- [x] **StereoSpeakerOutput (`stereo_speaker_output`)** — shipped 2026-07-02, closing the v0.1 docstring promise. A fourth speaker-family sink: `in_l`/`in_r` + `pan_cv`; constant-power (−3 dB centre) pan for mono sources, unity-centre cosine balance + mid/side `width` (0..2, skipped bit-exactly at 1) for pairs; per-sample autopan via `pan_cv` (house cv_depth convention); voice inputs sum at the jacks; drained by its own `_drain_stereo_speaker` pass onto the shared master bus. Defaults pass a stereo pair bit-exactly. 23 tests in `tests/test_stereo_speaker.py`. Example `examples/stereo_field_pluck.json`. Follow-ups: ~~a width_cv~~ (shipped 2026-07-02, same day — per-sample, shared `cv_depth` per the Reverb paired-CV convention, width==1 bit-exact skip preserved while the jack is silent, +8 tests → 31, example gains a 0.06 Hz width-breath LFO); per-sink meters on the node; a pan-law selector if anyone ever asks.
...
- [x] Stereo-aware speaker module (pan / width) — shipped 2026-07-02 as `stereo_speaker_output` (see below)
